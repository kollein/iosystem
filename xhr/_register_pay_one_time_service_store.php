...
if( is_array($cart_data_json) && count($cart_data_json) > 0 ){
	...
	$ajaxData_mix->insertRow(REGISTER_PAY_ONE_TIME_SERVICE_STORE, $NAME_VALUE[0], $NAME_VALUE[1], false);
    if( $ajaxData_mix->_rendata ){
    	$data_return['id'] = 1;
    	$data_return['success_alert'] = '<h1 class="center">Đăng ký thành công, chúng tôi sẽ liên hệ với bạn ngay.</h1>';
    }else{
